...
getProxyAgent_fn = async function() {
  __privateGet(this, _proxyAgent) || __privateSet(this, _proxyAgent, (await import("./index-zlExUSPL.js").then((n) => n.i)).HttpsProxyAgent);
  return __privateGet(this, _proxyAgent);
};
getFetch_fn = async function() {
  const hasWindow = typeof window !== "undefined" && !!window;
  __privateGet(this, _fetch) || __privateSet(this, _fetch, hasWindow ? window.fetch : (await import("./index-IT0cChwe.js")).default);
...
async function findRootFolder(client) {
  ...
  const data = await authorizedFetch(client, url2, { method: "GET" });
...
async function listAllInFolder(client, folderId) {
  const query = `'${folderId}' in parents and trashed=false`;
  const url2 = `${DRIVE_BASE_URL}?q=${encodeURIComponent(query)}&fields=files(id,name,mimeType),nextPageToken&supportsAllDrives=true&includeItemsFromAllDrives=true`;
  const data = await authorizedFetch(client, url2, {
    method: "GET"
  });
  return data.files || [];
...
async function downloadFolderRecursive(client, folderId, localPath) {
  const items = await listAllInFolder(client, folderId);
  console.log("Items found:", items.length);
  for (const item of items) {
    const itemPath = path$2.join(localPath, item.name);
    if (item.mimeType === "application/vnd.google-apps.folder") {
      if (!fs$4.existsSync(itemPath))
        fs$4.mkdirSync(itemPath);
      await downloadFolderRecursive(client, item.id, itemPath);
    } else {
      const data = await downloadFile(client, item.id);
      fs$4.writeFileSync(itemPath, data);
    }
...
async function syncServer(serverId) {
  ...
  try {
    if (!fs$4.existsSync(targetPath))
      fs$4.mkdirSync(targetPath, { recursive: true });
...
async function uploadFile(client, localFilePath, parentId) {
  var _a2;
  await refreshIfNeeded(client);
  const accessToken = client.credentials.access_token;
  const fileName = path$2.basename(localFilePath);
  const fileBuffer = fs$4.readFileSync(localFilePath);
  ...
  const existing = (_a2 = checkRes.files) == null ? void 0 : _a2[0];
  const metadata = JSON.stringify({ name: fileName, parents: existing ? void 0 : [parentId] });
...
--${boundary}--`)
  ...
  const uploadUrl = existing ? `https://www.googleapis.com/upload/drive/v3/files/${existing.id}?uploadType=multipart` : `https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart`;
...
async function uploadFolderRecursive(client, localPath, parentId) {
  var _a2, _b;
  const items = fs$4.readdirSync(localPath);
  for (const item of items) {
    const itemPath = path$2.join(localPath, item);
    const stat = fs$4.statSync(itemPath);
    if (stat.isDirectory()) {
      const query = `'${parentId}' in parents and name='${item}' and mimeType='application/vnd.google-apps.folder' and trashed=false`;
      const checkUrl = `${DRIVE_BASE_URL}?q=${encodeURIComponent(query)}&fields=files(id)`;
      const checkRes = await authorizedFetch(client, checkUrl, { method: "GET" });
      let folderId = (_b = (_a2 = checkRes.files) == null ? void 0 : _a2[0]) == null ? void 0 : _b.id;
      if (!folderId)
...
async function uploadWorldFolder(serverId, localWorldPath) {
  console.log("Uploading world folder:", localWorldPath);
  const client = getOAuthClient();
  if (!fs$4.existsSync(localWorldPath))
    return { success: false, error: "Local world folder not found." };
  try {
    await uploadFolderRecursive(client, localWorldPath, serverId);
    return { success: true };
  } catch (err) {
    return { success: false, error: err.message };
  }
...
ipcMain.handle("upload-world", async (_, serverId, localWorldPath) => {
  return await uploadWorldFolder(serverId, localWorldPath);
});
