...
getProxyAgent_fn = async function() {
  __privateGet(this, _proxyAgent) || __privateSet(this, _proxyAgent, (await import("./index-Dse4nIxY.js").then((n) => n.i)).HttpsProxyAgent);
  return __privateGet(this, _proxyAgent);
};
getFetch_fn = async function() {
  const hasWindow = typeof window !== "undefined" && !!window;
  __privateGet(this, _fetch) || __privateSet(this, _fetch, hasWindow ? window.fetch : (await import("./index-e-t4ZuVH.js")).default);
...
async function listAllInFolder(client, folderId) {
  const query = `'${folderId}' in parents and trashed=false`;
  const url2 = `${DRIVE_BASE_URL}?q=${encodeURIComponent(query)}&fields=files(id,name,mimeType,modifiedTime,size),nextPageToken&supportsAllDrives=true&includeItemsFromAllDrives=true`;
  const data = await authorizedFetch(client, url2, {
    method: "GET"
  });
  return data.files || [];
...
async function uploadServerFolder(serverId) {
  const client = getOAuthClient();
...
function shouldDownloadFile(localPath, driveFile) {
  if (!fs$4.existsSync(localPath))
    return true;
  const localStat = fs$4.statSync(localPath);
...
function shouldUploadFile(localFilePath, driveFile) {
  if (!driveFile)
    return true;
  const localStat = fs$4.statSync(localFilePath);
  if (localStat.size !== parseInt(driveFile.size))
    return true;
  const driveModified = new Date(driveFile.modifiedTime).getTime();
