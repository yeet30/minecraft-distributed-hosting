...
getProxyAgent_fn = async function() {
  __privateGet(this, _proxyAgent) || __privateSet(this, _proxyAgent, (await import("./index-UVAP8ErA.js").then((n) => n.i)).HttpsProxyAgent);
  return __privateGet(this, _proxyAgent);
};
getFetch_fn = async function() {
  const hasWindow = typeof window !== "undefined" && !!window;
  __privateGet(this, _fetch) || __privateSet(this, _fetch, hasWindow ? window.fetch : (await import("./index-QbMChkyK.js")).default);
...
async function listFilesInFolder(client, folderId) {
  const query = `'${folderId}' in parents and trashed=false`;
  const url2 = `${DRIVE_BASE_URL}?q=${encodeURIComponent(query)}&fields=files(id,name,mimeType)`;
  const data = await authorizedFetch(client, url2, {
    method: "GET"
...
async function syncServer(serverId) {
  ...
  try {
    const files2 = await listFilesInFolder(client, serverId);
    console.log("FILES FOR SYNC:", files2);
